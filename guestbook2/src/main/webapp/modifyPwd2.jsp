...
<%
    String uname = (String)session.getAttribute("admin"); //<!-- 获取会话属性admin的值，并将其转换为字符串 -->
    if(uname == null)
    {
    response.sendRedirect("admin-login.jsp"); //<!-- 如果admin属性为null，重定向到admin-login.jsp -->
    return; //<!-- 终止当前页面的执行 -->
    }
%>
...
<form name="f" method="post" action="RegServlet" onSubmit="return verify()"> <!-- 定义表单，名称为f，提交到RegServlet，使用post方法，提交时调用verify函数 -->
    <input type="hidden" name="from" value="modify2"/> <!-- 隐藏输入框，名称为from，值为modify2 -->
